from ISA import Register_port, read_code, Instruction, Instruction_Type, de_char, char, MATH_INSTRUCTION, \
...
class Register:
    def __init__(self, type: Register_port):
        self.name = type
        if type == Register_port.SP:
            self.value = 256
        else:
            self.value = 0
...
class DataPath:
    def __init__(self, size: int, inputfile: str):
        self.data_memory = [Cell()] * size
        self.instr_memory = [Cell()] * size
        self.stack = [0] * 256
        self.size = size
        self.input_buffer = Buffer(256)
        self.output_buffer = Buffer(256)
        self.registers = {
            '2': Register(Register_port.BR),
            '3': Register(Register_port.AC),
            '5': Register(Register_port.SP),
            '4': Register(Register_port.PS),
            '1': Register(Register_port.IP),
            '6': Register(Register_port.AR),
            '7': Register(Register_port.IR)
        }
        ...
        if inputfile != '':
            with open(inputfile) as f:
                ...
                for i in c:
                    ...
                    self.set_value_data_memory(self.input_index, char[i])
                    self.input_index += 1

    def set_value_data_memory(self, index: int, value: int):
        old_cell = self.data_memory[index]
        new_cell = Cell()
        new_cell.ins = old_cell.ins
        new_cell.value = value
        self.data_memory[index] = new_cell

    def set_value_instr_memory(self, index: int, value: int):
        old_cell = self.data_memory[index]
        new_cell = Cell()
        new_cell.ins = old_cell.ins
        new_cell.value = value
        self.instr_memory[index] = new_cell
    ...
    def get_value_data_memory(self, index: int) -> int:
        return self.data_memory[index].value

    def get_value_instr_memory(self, index: int) -> int:
        return self.instr_memory[index].value

    def print_registers(self):
        print("BR:%s, AC:%s, SP:%s, PS:%s, IP:%s, AR:%s, IR:%s" %
              (self.get_string_register('2'), self.get_string_register('3'),
               self.get_string_register('5'), self.get_string_register('4'), self.get_string_register('1'),
               self.get_string_register('6'), self.registers['7'].to_string()), end="")
...
class CPU:
    ...
    def decode(self):
        ...
        for i in self.program['Instruction']:
            new_cell = Cell()
            new_cell.ins = i
            self.datapath.instr_memory[end] = new_cell
            end += 1
        ...
        for i in self.program['Variable']:
            ...
            if self.program['Variable'][i].isdigit():
                ...
                new_cell = Cell()
                new_cell.value = v
                self.datapath.data_memory[end] = new_cell
                end += 1
            else:
                # else:
    ...
    def read_ins(self):
        # IP->AR
        self.alu.put_right(self.datapath.get_value_register('1'))
        r = self.alu.act(ALU.or_operation)
        self.datapath.set_value_register('6', r)
        self.tick_tick()
        # IP + 1 -> IP, [AR] -> IR
        self.datapath.set_value_register("1", self.datapath.get_value_register('1') + 1)
        self.tick_tick()
        self.datapath.set_value_register('7', self.datapath.instr_memory[self.datapath.get_value_register("6")].ins)

    def read_var(self, var: str):
        # VAR -> AR
        pos = self.var[var]
        self.alu.put_right(pos)
        self.datapath.set_value_register("6", self.alu.get_right())
        self.tick_tick()
        return self.datapath.get_value_data_memory(self.datapath.get_value_register("6"))
    # get proper value
    def addressing(self, ins: Instruction):
        arg = ins.args[0]
        if arg.isdigit():
            # Decoder -> AR
            self.datapath.set_value_register("6", int(arg))
            self.tick_tick()
            return self.datapath.get_value_data_memory(int(arg))
    ...
    def math(self, ins: Instruction, opr):
        ...
        self.alu.put_left(self.datapath.get_value_register("3"))
        self.tick_tick()
        if opr == ALU.div:
            # result ->　AC
            r = self.alu.act(opr)
            self.datapath.set_value_register("3", r)
            self.tick_tick()
        else:
            # result ->　AC
            result = self.alu.act(opr)
            self.datapath.set_value_register("3", result)
            self.tick_tick()
    ...
    def ins_execute(self, ins: Instruction, position: str) -> int:
        if ins.instruction == Instruction_Type['HLT']:
            ...
            if ins.instruction == Instruction_Type['ADD']:
                ...
                self.datapath.set_value_register("4", self.get_nzvc())
                ...
                self.alu.put_left(self.datapath.get_value_register("3"))
                self.tick_tick()
                self.datapath.set_value_register("3", self.alu.act(ALU.inversion))
                self.set_nzvc(Z)
                self.datapath.set_value_register("4", self.get_nzvc())
                self.tick_tick()
            # CMP
            ...
            if ins.instruction == Instruction_Type['LD']:
                ...
                if arg != 'INPUT':
                    self.datapath.set_value_register('3', self.addressing(ins))
                    self.tick_tick()
                elif arg == 'INPUT':
                    # index -> AR， io->ac
                    assert self.datapath.output_index < self.datapath.size, 'Read input out of range!'
                    self.datapath.set_value_register("6", self.datapath.output_index)
                    self.datapath.output_index += 1
                    self.tick_tick()
                    self.datapath.set_value_register("3",self.datapath.data_memory[self.datapath.get_value_register('6')].value)
                    self.tick_tick()
            # ST
            else:
                ...
                if arg != 'OUTPUT':
                    ...
                    self.datapath.set_value_register("6", self.alu.act(ALU.or_operation))
                    ## AC -> [AR]
                    self.datapath.set_value_data_memory(self.datapath.get_value_register("6"),
                                                        self.datapath.get_value_register("3"))
                    self.tick_tick()
                else:
                    # AC -> outputbuffer
                    self.datapath.output_buffer.buf[
                        self.datapath.output_buffer.write_pointer] = self.datapath.get_value_register('3')
            ...
            if ins.instruction == Instruction_Type.PUSH:
                # SP-1 -> SP
                self.datapath.set_value_register('5', self.datapath.get_value_register('5') - 1)
                self.tick_tick()
                # AC -> STACK[SP]
                self.datapath.stack[self.datapath.get_value_register("5")] = self.datapath.get_value_register("3")
                self.tick_tick()
            else:
                # [SP] -> AC
                self.datapath.set_value_register("3", self.datapath.stack[self.datapath.get_value_register("5")])
                self.tick_tick()
                # SP + 1 -> SP
                self.datapath.set_value_register('5', self.datapath.get_value_register('5') - 1)
                self.tick_tick()
            ...
            if ins.instruction == Instruction_Type.JMP:
                ...
                self.datapath.set_value_register("1", self.fun[position][arg])
                self.tick_tick()
            elif ins.instruction == Instruction_Type.CALL:
                # AC->BR save parameter
                self.alu.put_left(self.datapath.get_value_register('3'))
                self.datapath.set_value_register("2", self.alu.act(ALU.or_operation))
                self.tick_tick()
                # IP ->　AC
                arg = ins.args[0]
                assert arg in self.fun.keys(), "You are trying call a not existed function"
                self.alu.put_right(self.datapath.get_value_register('1'))
                self.datapath.set_value_register("3", self.alu.act(ALU.or_operation))
                self.tick_tick()
                # push
                new_ins = Instruction(Instruction_Type.PUSH, [])
                self.ins_execute(new_ins, position)
                # Decoder -> IP
                self.datapath.set_value_register("1", self.fun[arg]['self'])
                self.position.append(arg)
                self.tick_tick()
                # BR->AC save parameter
                self.alu.put_left(self.datapath.get_value_register('2'))
                self.datapath.set_value_register("3", self.alu.act(ALU.or_operation))
                self.tick_tick()
            elif ins.instruction == Instruction_Type.RET:
                # AC->BR make sure that result of function is saved
                self.datapath.set_value_register("2", self.datapath.get_value_register("3"))
                self.tick_tick()
                # pop
                ...
                # AC -> IP
                self.alu.put_left(self.datapath.get_value_register("3"))
                self.datapath.set_value_register("1", self.alu.act(ALU.or_operation))
                self.tick_tick()
                # BR->AC
                self.datapath.set_value_register("3", self.datapath.get_value_register("2"))
                self.tick_tick()
            elif ins.instruction == Instruction_Type.JZ:
                if self.datapath.get_value_register("4") | Z == Z and self.datapath.get_value_register("4") != 0:
                ...
                if self.datapath.get_value_register("4") | N == N and self.datapath.get_value_register("4") != 0:
                    ins_2 = Instruction(Instruction_Type.JMP, args=ins.args)
                    self.ins_execute(ins_2, position)
            # JNZ
            else:
                if self.datapath.get_value_register("4") != Z:
                    ins_2 = Instruction(Instruction_Type.JMP, args=ins.args)
                    self.ins_execute(ins_2, position)
        return 0
    ...
    def run_ins(self, position: str) -> int:
        ins: Instruction
        self.read_ins()
        ins = self.datapath.get_value_register("7")
        result = self.ins_execute(ins, position)
        self.instr_instr()
        print("DEBUG:root:{ ", end="")
        print("instr:{}".format(self.instr), end=", ")
    ...
    def run(self):
        ...
        if output_result:
            ...
            return str(self.datapath.get_value_register("3"))
